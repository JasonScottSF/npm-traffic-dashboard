...
**Database stats & VACUUM** — the Ops tab includes a Database section showing total Postgres size, date range of stored request data, a per-table row count and size breakdown, and a Run VACUUM ANALYZE button with loading state.

**Mobile-responsive layout** — wide tables are wrapped in horizontal scroll containers; less-important columns (paths, last-seen, user agent) are hidden on small screens; stat card grids stack to 2 columns on mobile; chart containers have a minimum height to prevent collapse.
...
Configurable alert rules are managed from the Ops tab → Alerts section. Each rule specifies a condition, optional parameters, a delivery channel (email, webhook, Slack), and a cooldown.

| Condition | What it checks | Key parameters |
|-----------|---------------|----------------|
| `ssl_cert_expiry` | SSL cert expiring within N days | `days` (default 30) |
| `container_down` | Docker container not in running state | `name` (optional, blank = any) |
| `breach_events` | Unacknowledged WAF bypass events ≥ N | `threshold` (default 1) |
| `error_rate` | 5xx error rate exceeds N% in a window | `threshold`, `window_minutes` |
| `host_down` | Proxy host health check failing | `host` (optional, blank = any) |
